...
def main():
    ...
    # SHAP Analysis (only for supported models)
    try:
        # Use test set for SHAP analysis if available
        if hasattr(pipeline_runner.training_pipeline, 'X_test') and pipeline_runner.training_pipeline.X_test is not None:
            X_shap = pipeline_runner.training_pipeline.X_test
        else:
            X_shap = None
        if X_shap is not None:
            print("\nRunning SHAP analysis and saving outputs...")
            shap_analyzer = SHAPAnalyzer(model, X_shap, model_type='tree', output_dir='shap_outputs')
            shap_analyzer.run_full_analysis(features_for_dependence=[X_shap.columns[0], X_shap.columns[1]])
            print("SHAP analysis complete. Outputs saved to 'shap_outputs/' directory.")
        else:
            print("\nSHAP analysis skipped: No test set available.")
