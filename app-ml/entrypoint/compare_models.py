...
def train_and_evaluate_model(model_type, config, engineered_data):
    ...
    try:
        ...
        # Prepare features and target for CV and learning curve
        df_with_target = training_pipeline.create_target_variable(engineered_data)
        X, y = training_pipeline.prepare_features_and_target(df_with_target)

        # Cross-validation (5-fold)
        print(f"\nPerforming 5-fold cross-validation for {model_type}...")
        from pipelines.model_factory import ModelFactory
        model_for_cv = ModelFactory.create_model(model_type, training_pipeline.model_params)
        cv_scores = cross_val_score(model_for_cv, X, y, cv=5, scoring='r2', n_jobs=-1)
        print(f"CV R2 scores: {cv_scores}")
        print(f"Mean CV R2: {np.mean(cv_scores):.4f}, Std: {np.std(cv_scores):.4f}")

        # Learning curve
        print(f"Plotting learning curve for {model_type}...")
        train_sizes, train_scores, test_scores = learning_curve(
            model_for_cv, X, y, cv=5, scoring='r2', n_jobs=-1,
        ...
        plt.legend()
        plt.tight_layout()
        plot_dir = project_root / 'models' / 'experiments' / 'learning_curves'
        plot_dir.mkdir(parents=True, exist_ok=True)
        plot_path = plot_dir / f'learning_curve_{model_type}.png'
        ...
        result = {
            ...
            'Learning_Curve_Path': str(plot_path),
            'Status': 'Success'
        }
